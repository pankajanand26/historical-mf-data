...
const TIP = { backgroundColor: '#fff', border: '1px solid #1e3a5f30', borderRadius: 6, fontSize: 11 };
...
const ReturnsSection = ({ data }) => {
  ...
  return (
    <div>
      ...
      <div className="bg-white rounded-xl border border-editorial-navy/10 p-5 mb-6">
        <ResponsiveContainer width="100%" height={300}>
          <LineChart data={chartData} margin={{ top: 5, right: 10, bottom: 5, left: 0 }}>
            <CartesianGrid strokeDasharray="3 3" stroke="#1e3a5f10" />
            <XAxis dataKey="date" tickFormatter={tickFormatter} tick={{ fontSize: 10, fill: '#1e3a5f60' }} tickLine={false} />
            <YAxis tickFormatter={(v) => `${v.toFixed(1)}%`} tick={{ fontSize: 10, fill: '#1e3a5f60' }} tickLine={false} axisLine={false} />
            ...
            {(data?.funds ?? []).map((f, i) => (
              <Line key={f.scheme_code} type="monotone" dataKey={`fund_${f.scheme_code}`} name={f.scheme_name} stroke={FUND_COLORS[i % FUND_COLORS.length]} dot={false} strokeWidth={2} />
            ))}
          </LineChart>
        </ResponsiveContainer>
      </div>
      ...
      <div className="bg-white rounded-xl border border-editorial-navy/10 overflow-hidden">
    ...
    <div className="space-y-8">
      <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          <div className="bg-white rounded-xl border border-editorial-navy/10 overflow-hidden">
            <div className="px-5 py-3 border-b border-editorial-navy/10"><p className="font-serif text-base text-editorial-navy font-semibold">Volatility & Tracking</p></div>
          ...
          <div className="bg-white rounded-xl border border-editorial-navy/10 overflow-hidden">
            <div className="px-5 py-3 border-b border-editorial-navy/10"><p className="font-serif text-base text-editorial-navy font-semibold">Risk-Adjusted Returns</p></div>
      ...
      <div>
        ...
        <div className="bg-white rounded-xl border border-editorial-navy/10 p-5">
          <ResponsiveContainer width="100%" height={260}>
            <ScatterChart margin={{ top: 10, right: 20, bottom: 20, left: 0 }}>
              <CartesianGrid strokeDasharray="3 3" stroke="#1e3a5f10" />
              <XAxis type="number" dataKey="x" tickFormatter={(v) => `${v.toFixed(1)}%`} tick={{ fontSize: 10, fill: '#1e3a5f60' }} label={{ value: 'Std Dev', position: 'insideBottom', offset: -10, fontSize: 10, fill: '#1e3a5f60' }} />
              <YAxis type="number" dataKey="y" tickFormatter={(v) => `${v.toFixed(1)}%`} tick={{ fontSize: 10, fill: '#1e3a5f60' }} tickLine={false} axisLine={false} />
          ...
          <div className="bg-white rounded-xl border border-editorial-navy/10 p-5">
            <ResponsiveContainer width="100%" height={240}>
              <ScatterChart margin={{ top: 10, right: 10, bottom: 20, left: 0 }}>
                <CartesianGrid strokeDasharray="3 3" stroke="#1e3a5f10" />
                <XAxis type="number" dataKey="x" domain={domain} tickFormatter={(v) => `${v.toFixed(0)}%`} tick={{ fontSize: 9, fill: '#1e3a5f60' }} label={{ value: 'Benchmark', position: 'insideBottom', offset: -10, fontSize: 9, fill: '#1e3a5f60' }} />
                <YAxis type="number" dataKey="y" domain={domain} tickFormatter={(v) => `${v.toFixed(0)}%`} tick={{ fontSize: 9, fill: '#1e3a5f60' }} tickLine={false} axisLine={false} />
                ...
                <ReferenceLine segment={[{ x: domain[0], y: domain[0] }, { x: domain[1], y: domain[1] }]} stroke="#1e3a5f20" strokeDasharray="4 2" />
        ...
        <div>
          ...
          <div className="space-y-4">
            {allStats.map(({ fund, color, alphaData }) => {
              ...
              return (
                <div key={fund.scheme_code} className="bg-white rounded-xl border border-editorial-navy/10 p-4">
                  <p className="font-serif text-xs text-editorial-navy font-semibold mb-2">{shortName(fund.scheme_name)}</p>
                  ...
                  <ResponsiveContainer width="100%" height={120}>
                    <AreaChart data={alphaData} margin={{ top: 3, right: 5, bottom: 3, left: 0 }}>
                      <CartesianGrid strokeDasharray="3 3" stroke="#1e3a5f10" />
                      <XAxis dataKey="date" tickFormatter={tickFormatter} tick={{ fontSize: 8, fill: '#1e3a5f60' }} tickLine={false} />
                      <YAxis tickFormatter={(v) => `${v.toFixed(0)}%`} tick={{ fontSize: 8, fill: '#1e3a5f60' }} tickLine={false} axisLine={false} />
                      <Tooltip contentStyle={TIP} formatter={(v) => fmt2(v)} />
                      <ReferenceLine y={0} stroke="#1e3a5f20" strokeDasharray="4 2" />
                      <Area type="monotone" dataKey="alpha" stroke={color} fill={`url(#egrad-${fund.scheme_code})`} strokeWidth={1.5} dot={false} />
                    </AreaChart>
                  </ResponsiveContainer>
                </div>
              );
